...
// RELATÓRIO 4: Lista de Veículos (MELHORADO)
...
// RELATÓRIO 3: Despesas Diretas de Veículos (CORRIGIDO E MELHORADO)
...
// RELATÓRIO 2: Despesas de Frota (Rateado) (CORRIGIDO)
apiRouter.get('/relatorios/custoRateado', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const finalData = filial ? data.filter(d => d.filial_id == filial) : data;

        res.json(finalData);
    } catch (error) {
        console.error("Erro ao gerar relatório de despesas rateadas:", error);
        res.status(500).json({ error: 'Erro ao gerar relatório.' });
    } finally {
        if (connection) await connection.end();
    }
...
// RELATÓRIO 1: Despesa Total por Filial (CORRIGIDO)
apiRouter.get('/relatorios/custoTotalFilial', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (filial) {
            combinedData = combinedData.filter(d => d.filial_id == filial);
        }
        ...
        console.error("Erro ao gerar relatório de despesa total:", error);
        res.status(500).json({ error: 'Erro ao gerar relatório.' });
    } finally {
        if (connection) await connection.end();
    }
