...
// Exportar utilitários para as telas de impressão poderem usar
window.limpaCod = function(cod) { return cod ? cod.toString().replace(/^0+(?=\d)/, '') : ''; };

window.formatarEnderecoCompleto = function(end) {
...
window.extrairObservacao = function(obsRaw) {
    if (!obsRaw) return '';
...
window.abrirVisualizacaoRomaneio = async function(id) {
    ...
    try {
        ...
        // Botões de impressão agora chamam as funções do impressao_a4.js
        if(window.imprimirRoteiro) document.getElementById('btn-imprimir-romaneio').onclick = () => window.imprimirRoteiro(data.id);
        ...
        if(window.imprimirPedidosCarga) btnImprimirDavs.onclick = () => window.imprimirPedidosCarga(data.id);
            ...
            dados.itens.forEach(item => {
                ...
                html += `
                    <div class="p-3 flex justify-between items-center hover:bg-gray-50 transition-colors">
                        <div>
                            <p class="text-sm font-bold text-gray-800">${window.limpaCod(item.produto_codigo)} - ${item.produto_nome} ${tagDev}</p>
                            ${obsModal}
                        </div>
                        ...
                        ${itens.map(item => {
                            ...
                            <tr class="expandable-row hover:bg-gray-50 transition-colors" data-idavs-regi="${item.idavs_regi}">
                                <td style="border:none;" class="px-3 py-3 font-medium text-gray-800">
                                    ${window.limpaCod(item.pd_codi)} - ${item.pd_nome} <span class="text-gray-400 text-xs ml-1">(${item.unidade})</span> 
                                    ${tagDev}
                                    ${obsBalcao}
                                </td>
                ...
                <div class="flex justify-between items-start border-b border-gray-100 pb-5 mb-5">
                    ...
                    <div class="text-right">
                        ...
                        <div class="flex gap-2 justify-end">
                            <button onclick="if(window.imprimirEspelhoDav) window.imprimirEspelhoDav('${data.dav_numero}')" class="action-btn bg-gray-100 text-gray-700 border border-gray-300 hover:bg-gray-200 text-xs py-1.5 px-3 flex inline-flex items-center gap-1.5 shadow-sm">
                                <i data-feather="printer" class="w-3.5 h-3.5"></i> Espelho (A4)
                            </button>
                            <button onclick="if(window.imprimirCupomTermico) window.imprimirCupomTermico('${data.dav_numero}')" class="action-btn bg-yellow-100 text-yellow-800 border border-yellow-300 hover:bg-yellow-500 hover:text-white text-xs py-1.5 px-3 flex inline-flex items-center gap-1.5 shadow-sm">
...
function renderPendingList() {
    ...
    container.innerHTML = davsVisiveis.map(dav => {
        ...
        if (dav.nota_fiscal && dav.chave_nfe) {
            nfeBadge = `<button onclick="window.abrirDanfe('${dav.chave_nfe}')" class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-[9px] font-bold uppercase border border-blue-200 hover:bg-blue-600 hover:text-white transition-colors" title="Ver Nota Fiscal">NFe ${dav.nota_fiscal}</button>`;
        }
        ...
        const itensHtml = dav.itens.map(item => {
            ...
            const qtdDev = parseFloat(item.devolvido || item.quantidade_devolvida || item.it_qtdv || 0);
            ...
            <div class="flex flex-col border-b border-indigo-100/50 py-1.5 last:border-0 hover:bg-indigo-50 px-1 rounded transition-colors">
                <div class="flex justify-between items-center">
                    <span class="text-[11px] font-medium text-gray-700 truncate flex-1 pr-2">${window.limpaCod(item.codigo)} - ${item.nome} ${tagDev}</span>
...
function renderCartList() {
    ...
    container.innerHTML = cartDavs.map(dav => {
        const itensCartHtml = dav.itens.map(item => {
            // Etiqueta de devolução no Carrinho
            const qtdDev = parseFloat(item.devolvido || item.quantidade_devolvida || item.it_qtdv || 0);
            ...
            <div class="flex justify-between items-center mt-1.5 border-t border-gray-100 pt-1.5">
                <span class="text-[10px] font-bold text-gray-700 truncate flex-1">${window.limpaCod(item.codigo)} - ${item.nome} ${tagDev}</span>
...
async function finalizarCarga() {
    ...
    const executaFinalizacao = async () => {
        ...
        try {
            ...
            if(window.imprimirPedidosCarga) window.imprimirPedidosCarga(romaneioId); 
            switchView('romaneio-list-view');
        ...
        dados.itens.forEach(item => {
            ...
            html += `
                <div class="p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-3 transition-colors ${rowClass}">
                    <div class="flex-1"><p class="text-sm font-bold text-gray-800">${window.limpaCod(item.produto_codigo)} - ${item.produto_nome}</p><p class="text-[10px] font-black text-gray-500 mt-1 uppercase tracking-wider">Enviado: <span class="text-indigo-600 text-xs">${state.qtd_enviada}</span> ${item.produto_unidade} ${feedbackHtml ? `&nbsp;&bull;&nbsp; ${feedbackHtml}` : ''}</p></div>
